...
**产出物不得反向依赖本 skill**：由本方法论创建或维护的项目，其自足性不得以"本 skill 在场"为前提，项目文件中也不出现本 skill 的名字（唯一例外是显式声明的档 C，见 Seed §三档自足）。
...
- **生成物不得反向依赖生成器**：项目的自足性不以任何外部方法论在场为前提（见 Seed §三档自足）。
...
**关键约束——生成物不得反向依赖生成器**。判据：*删掉项目里指向外部方法论的那句话，项目是否仍然完整？*

- 仍完整 ⇒ 那是**指针**（可选的升级路径），合规
- 不完整 ⇒ 那是**依赖**，要么把规则内联，要么按档 C 显式声明

**三档自足**（项目在 CLAUDE.md/AGENTS.md 中声明自己属哪一档）：

| 档 | 含义 | 要求 |
|----|------|------|
| **A** | 完全自足 | 规则全部内联，不指向任何外部方法论 |
| **B**（默认，Initialize 产出） | 自足 + 可选升级路径 | 核心规则内联；可有指针，但删掉指针项目仍完整 |
| **C** | 显式依赖外部方法论 | **必须写明**：依赖什么、缺失时会失去哪些能力、如何获得。依赖描述不得写机器绝对路径 |

档 C 是正当选择（例如项目确定始终与某套方法论一起使用），但必须是**声明过的**选择——而不是让下游对着一句含混的指针各自猜测。

**种子内容**：自足声明、项目文件即记忆、log 追加规则、Resume 阅读协议、frontmatter 基本要求、双文件同步、感知偏差与对齐。

**种子模板**（模板版本：**2026-08-13**）：
...
（若本项目确实依赖某套外部方法论，改写此行并写明：依赖什么、缺失时会失去哪些能力、如何获得。不写机器绝对路径。）
...
种子块**不含任何指向本 skill 的页脚或署名**。项目若要保留升级路径（档 B），由项目自行写一行中性指针，且须满足上面的"删掉它仍完整"判据。
...
| 自足档位声明 | 声明了 A/B/C，且实际内容与之一致（A/B 无必需的外部依赖；C 写明依赖与缺失代价） | 建议补声明或改正 |
