...
**任务较重的项目**：复杂事项拆结构化任务资产（需求/边界 · 上下文 · 实施清单 · 决策记录，分区或分 `{task}/` 目录），简单事项 inline。这是**轻量任务管理**——借结构、不借重编排/hook 注入（见 Boundaries）。
...
- 重编排/多 agent 调度器（多角色分工、每轮 workflow-state 注入等用 task-forest / Trellis 等专门工具）——但轻量结构化任务资产（复杂事项拆 需求/设计/实施清单/决策记录）是标准项目的一等组成，见 Customization
- git 的替代品（版本管理就是 git）
- 强制模板（最小约束，项目自行演化）
